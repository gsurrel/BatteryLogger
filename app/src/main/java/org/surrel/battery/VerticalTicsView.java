...
public class VerticalTicsView extends View
{
    ...
    public void onDraw(Canvas canvas)
    {
        ...
        float min = (float) (Math.floor(value_min / value_step + 0.5f) * value_step);
        float max = (float) (Math.ceil(value_max / value_step + 0.5f) * value_step);
        for(float v = 0; min + v < max; v += value_step) {
            ...
            canvas.drawText(str, (float) w, ty, paint);
        }
    }
